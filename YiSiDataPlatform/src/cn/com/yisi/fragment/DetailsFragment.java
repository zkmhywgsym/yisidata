...
public class DetailsFragment extends BaseFragment {
	...
	private List<NameValuePair> getParams() {
		...
		// params.add(new BasicNameValuePair("carNum",
		// i.getStringExtra("carNum")));
		// params.add(new BasicNameValuePair("materiel",
		// i.getStringExtra("materiel")));
		// params.add(new BasicNameValuePair("status",
		// i.getStringExtra("status")));
		// params.add(new BasicNameValuePair("person",
		// i.getStringExtra("person")));
	...
	private class MyListAdapter extends BaseAdapter {
		...
		public View getView(int index, View view, ViewGroup root) {
			...
			holder.carNumLabel.setText(getResources().getString(
					R.string.details_carCode));
			if (Constants.FLAG_RECEIVE.equals(getValue(
					mainActivity.getIntent(), Constants.FLAG))) {
			...
			} else if (Constants.FLAG_SEND.equals(getValue(
					mainActivity.getIntent(), Constants.FLAG))) {
				holder.providerLabel.setText(getResources().getString(
...
//				new Date()));
		...
		AsyncTask<Void, Integer, List<EntityDetails>> task = new AsyncTask<Void, Integer, List<EntityDetails>>() {
			...
			protected List<EntityDetails> doInBackground(Void... arg0) {
				WebHelper<EntityDetails> helper = new WebHelper<EntityDetails>(
						EntityDetails.class);
	...
	private void statistics() {
		AsyncTask<Void, Integer, EntityStatistics> task = new AsyncTask<Void, Integer, EntityStatistics>() {
			...
			protected EntityStatistics doInBackground(Void... arg0) {
				WebHelper<EntityStatistics> helper = new WebHelper<EntityStatistics>(
						EntityStatistics.class);
				List<NameValuePair> params = getParams();
				return helper.getObject(Constants.URL_DETAILS_STATISTICS,
						params);
			}
